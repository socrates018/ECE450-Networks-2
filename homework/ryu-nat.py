        ...
        if dpid == 0x1A:
            if ethertype == ether_types.ETH_TYPE_ARP: 
                ...
                if out_port == 3:
                    ...
                    # Use add_nat to get (ROUTER1_TOP_IP, external_port)
                    ROUTER1_TOP_IP, external_port = self.add_nat(ip_pkt.src, udp_pkt.src_port)
                    self.logger.info(f"NAT table updated: ({ip_pkt.src}, {udp_pkt.src_port}) -> ({ROUTER1_TOP_IP}, {external_port})")
